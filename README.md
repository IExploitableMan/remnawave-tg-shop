...
-   **Базовый и улучшенный тарифы:** Поддерживается покупка базового тарифа, покупка улучшенного тарифа (`base + add-on`) и апгрейд активной базы до улучшенного тарифа.
-   **Докупка трафика add-on:** Для улучшенного тарифа доступны отдельные пакеты трафика с уведомлениями о скором окончании и исчерпании лимита.
-   **Отчёты о проблемах с серверами:** Пользователь может выбрать тип проблемы, отметить доступные именно ему серверы и отправить отчёт администраторам. Для сценариев «не открываются сайты» и «другая проблема» можно приложить текстовые детали.
...
-   **Промокоды:** Возможность применять промокоды для получения скидок, бонусных дней и ваучеров на трафик add-on.
    -   Промокоды могут действовать отдельно на базовую подписку, улучшенный тариф, combined-покупку и докупку трафика.
    -   Для скидок поддерживаются лимит максимальной скидки, ограничения по дате регистрации пользователя, режимы `только продление` / `только без активной подписки`, а также отдельное правило для комбинированного тарифа: скидка только на базовую часть или на всю покупку.
-   **Реферальная программа:** Пользователи могут приглашать друзей и получать за это бонусные дни подписки.
    -   Реферальная программа применяется только к базовой подписке и не распространяется на add-on.
...
-   **Ручная выдача тарифов:** Можно вручную выдать базовый тариф, улучшенный тариф и добавить трафик для add-on из админки.
...
-   **Runtime-настройки из бота:** Часть настроек можно менять прямо из админки. Они сохраняются и переживают перезапуск.
-   **Отчёты о серверах:** Админ получает уведомления о жалобах, может включать/выключать такие уведомления для себя, а также смотреть агрегированную статистику и список отчётов.
...
-   **Base plan** — обычная подписка VPN без дополнительных whitelist-маршрутов.
-   **Upgraded plan / add-on** — отдельный дополнительный профиль Remnawave с собственными squad-ами, сроком действия, трафиком и отдельной ссылкой подключения.
-   **Combined purchase** — покупка `base + add-on` сразу одним действием.
-   **Add-on top-up** — докупка трафика для add-on пакетами.

### Как устроен add-on

-   Для add-on ведутся отдельно:
    -   срок действия,
    -   включённый месячный трафик,
    -   остаток докупленных пакетов,
    -   отдельная ссылка подключения.
-   Если базовая подписка закончилась раньше, add-on уходит в suspended и автоматически оживает после продления базы, если срок add-on ещё не истёк.
-   Расход трафика идёт так:
    1.  сначала месячный включённый лимит,
    2.  затем докупленные top-up пакеты.
    3.  в конце срока месячный трафик сгорает, докупленные пакеты нет. 
    4.  докупленные пакеты сгорают в случае окончания действия add-on.

## ⚙️ Runtime settings

Часть настроек по-прежнему задаётся через `.env`, но для некоторых уже есть слой runtime-конфига:

-   значения из `.env` используются как **дефолты**;
-   изменённые через админку значения сохраняются
-   такие значения сохраняются после перезапуска контейнера;
-   архитектура рассчитана на постепенный перенос и других настроек из `.env` в runtime-конфиг.

На текущий момент через админку можно менять предупреждения о скором окончании:

-   базовой платной подписки,
-   пробного периода,
-   улучшенного тарифа.
...
2.  **Создайте и настройте файл `.env`:**
    ...
    | `SERVER_STATUS_URL` | (Опционально) Ссылка на внешнюю страницу статуса/мониторинга. | `https://status.example.com/your-service` |
    ...
    | `SERVER_REPORT_COOLDOWN_HOURS` | Минимальный интервал между пользовательскими отчётами о проблемах с серверами. | `24` |
    | `USER_HWID_DEVICE_LIMIT` | Лимит устройств (HWID) для новых panel users (`0` = безлимит). | `0` |
    | `REQUIRED_CHANNEL_SUBSCRIBE_TO_USE` | Включить/выключить обязательную проверку подписки на канал (`true`/`false`). | `false` |
    | `REQUIRED_CHANNEL_SUBSCRIPTION_MODE` | Режим проверки канала: `immediate` требует подписку сразу, `after_paid_subscription` включает требование только после появления платной подписки. | `immediate` |
    ...
    | Переменная | Описание |
    | --- | --- |
    | `WEBHOOK_BASE_URL`| **Обязательно** для webhook-режима. Базовый URL для вебхуков, например `https://your.domain.com`. |
    ...
    | `TELEGRAM_USE_POLLING` | Включить Telegram polling вместо webhook (`true`/`false`). Полезно для локальной отладки или временного запуска второго бота. |
    ...
    <details>
    <summary><b>Настройки улучшенного тарифа (add-on)</b></summary>

    | Переменная | Описание |
    | --- | --- |
    | `ADDON_RUB_PRICE_1_MONTH` | Доплата за add-on на 1 месяц в RUB. Используется как часть улучшенного тарифа и для отдельного апгрейда поверх базы. |
    | `ADDON_STARS_PRICE_1_MONTH` | Доплата за add-on на 1 месяц в Stars. |
    | `ADDON_MONTHLY_TRAFFIC_GB` | Включённый месячный трафик add-on. |
    | `ADDON_USER_SQUAD_UUIDS` | Internal squad UUID для add-on пользователя в Remnawave. |
    | `ADDON_USER_EXTERNAL_SQUAD_UUID` | Опциональный external squad UUID для add-on пользователя. |
    | `ADDON_TRAFFIC_PACKAGES` | Пакеты докупки трафика в формате `<GB>:<price>,<GB>:<price>`. |
    | `ADDON_STARS_TRAFFIC_PACKAGES` | Пакеты докупки трафика в Stars в формате `<GB>:<price>,<GB>:<price>`. |
    | `ADDON_TRAFFIC_WARNING_PERCENT` | Порог предупреждения по проценту оставшегося трафика. |
    | `ADDON_TRAFFIC_WARNING_GB` | Порог предупреждения по абсолютному остатку трафика в ГБ. |
    | `ADDON_TRAFFIC_WORKER_INTERVAL_SECONDS` | Интервал фонового воркера контроля трафика add-on. |
    </details>

    <details>
    <summary><b>Настройки предупреждений об окончании подписок</b></summary>

    Эти переменные задают стартовые значения. После запуска их можно менять из админки, и тогда фактические значения будут браться из БД.

    | Переменная | Описание |
    | --- | --- |
    | `EXPIRY_WARNING_WORKER_INTERVAL_SECONDS` | Интервал фонового воркера проверки сроков подписок. |
    | `BASE_EXPIRY_WARNING_ENABLED` | Включить предупреждение о скором окончании обычной платной базовой подписки. |
    | `BASE_EXPIRY_WARNING_HOURS_BEFORE` | За сколько часов предупреждать о скором окончании базовой подписки. |
    | `TRIAL_EXPIRY_WARNING_ENABLED` | Включить предупреждение о скором окончании trial. |
    | `TRIAL_EXPIRY_WARNING_HOURS_BEFORE` | За сколько часов предупреждать о скором окончании trial. |
    | `ADDON_EXPIRY_WARNING_ENABLED` | Включить предупреждение о скором окончании add-on / upgraded plan. |
    | `ADDON_EXPIRY_WARNING_HOURS_BEFORE` | За сколько часов предупреждать о скором окончании add-on. |
    </details>
    ...
    | `TRIAL_TRAFFIC_STRATEGY` | Стратегия сброса трафика пробного периода (`NO_RESET`, `WEEK`, `MONTH`). |
    </details>
...
4.  **Настройка вебхуков (Обязательно для webhook-режима Telegram):**
    ...
    **Пути для перенаправления:**
    ...
    -   **Для Telegram:** Бот автоматически установит вебхук, если в `.env` указан `WEBHOOK_BASE_URL` и `TELEGRAM_USE_POLLING=false`. Путь берётся из `TELEGRAM_WEBHOOK_PATH` (по умолчанию `https://<ваш_домен>/webhook/telegram`).

    Где `remnawave-tg-shop` — это имя сервиса из `docker-compose.yml`, а `<WEB_SERVER_PORT>` — порт, указанный в `.env`.

    > Если `TELEGRAM_USE_POLLING=true`, Telegram webhook не нужен, но webhook-и панели и платёжных систем всё равно остаются рабочими через встроенный web server.
...
5.  **Просмотр логов:**
    ...
    > 💡 Если включены server reports, пользователю будут показаны только те хосты/серверы Remnawave, к которым у него реально есть доступ по активным профилям base/add-on.
...
- `WEBHOOK_BASE_URL` - Обязательно для Telegram webhook-режима. Базовый URL для вебхуков, например `https://webhook.domain.com`
...
**Если используете улучшенный тариф:**
- `ADDON_RUB_PRICE_1_MONTH`
- `ADDON_MONTHLY_TRAFFIC_GB`
- `ADDON_USER_SQUAD_UUIDS`
- `ADDON_TRAFFIC_PACKAGES`
